...
const BgDecor = ({ accentColor }) => (
  ...
  >
    ...
    {/* Chấm trang trí */}
    <circle cx="20%" cy="75%" r="6" fill={accentColor} fillOpacity="0.18" />
    <circle cx="78%" cy="20%" r="9" fill={accentColor} fillOpacity="0.14" />
    <circle cx="88%" cy="55%" r="5" fill={accentColor} fillOpacity="0.20" />
    <circle cx="12%" cy="45%" r="7" fill={accentColor} fillOpacity="0.15" />
    {/* Hoa nhỏ — 4 cánh đơn giản */}
    <g transform="translate(92,60)" opacity="0.18">
      <ellipse cx="0" cy="-8" rx="4" ry="7" fill={accentColor} />
      <ellipse cx="0" cy="8" rx="4" ry="7" fill={accentColor} />
      <ellipse cx="-8" cy="0" rx="7" ry="4" fill={accentColor} />
      <ellipse cx="8" cy="0" rx="7" ry="4" fill={accentColor} />
      <circle cx="0" cy="0" r="4" fill="#fff" />
    </g>
    <g transform="translate(1100,320)" opacity="0.14">
      <ellipse cx="0" cy="-10" rx="5" ry="9" fill={accentColor} />
      <ellipse cx="0" cy="10" rx="5" ry="9" fill={accentColor} />
      <ellipse cx="-10" cy="0" rx="9" ry="5" fill={accentColor} />
      <ellipse cx="10" cy="0" rx="9" ry="5" fill={accentColor} />
...
/* ── Item sản phẩm ── */
const ProductItem = ({ item, index, visible, t }) => {
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-4 rounded-2xl px-5 py-4 cursor-pointer hover:-translate-y-1"
      style={{
        background: t.cardBg,
    ...
    >
      {/* Ảnh */}
      <div
        className="relative w-[76px] h-[76px] flex-shrink-0 rounded-xl overflow-hidden"
      ...
      {/* Divider dọc */}
      <div
        className="self-stretch w-px mx-1 flex-shrink-0"
...
const PopularSection = () => {
  ...
  const t = theme?.sectionTheme?.popularSection ?? {
    pageBg: "#FFF1F5",
    accentColor: "#F7a3a9",
    textPrimary: "#2d3748",
  ...
  return (
    <section
      ...
      style={{ background: t.pageBg }}
    >
      <BgDecor accentColor={t.accentColor} />
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-9">
          ...
          >
            <div className="relative w-full rounded-3xl overflow-hidden shadow-lg group lg:sticky lg:top-24 aspect-[3/4] sm:aspect-[16/9] lg:aspect-auto lg:h-full lg:min-h-[420px] transition-all duration-500 ease-out hover:-translate-y-1 hover:shadow-xl">
              <Image
                src={bannerImg}
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(to top, ${t.accentColor}30, transparent)`,
                }}
              />
              {/* Label */}
              <div className="absolute bottom-5 left-5 right-5">
                <span
                  className="inline-block backdrop-blur-sm font-bold text-sm px-4 py-2 rounded-full shadow-sm"
                  style={{ background: `${t.cardBg}e6`, color: t.accentColor }}
                >
                  🌟 Bán chạy nhất tháng
                </span>
              </div>
