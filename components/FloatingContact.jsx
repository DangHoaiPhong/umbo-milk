...
const MessengerIcon = ({ size = 20 }) => (
  ...
  >
    ...
    <path
      d="M24 8C15.163 8 8 14.71 8 23c0 4.556 1.99 8.636 5.2 11.54V40l4.93-2.71C19.954 37.74 21.94 38 24 38c8.837 0 16-6.71 16-15S32.837 8 24 8zm1.6 20.2l-4.08-4.35-7.96 4.35 8.76-9.3 4.18 4.35 7.86-4.35-8.76 9.3z"
      fill="white"
    />
  </svg>
);
...
const DESKTOP_ITEMS = [
  ...
  {
    id: "email",
    label: "Đăng ký & để lại lời nhắn",
    sub: "hello@umbomilk.com",
  ...
  {
    id: "maps",
    label: "Xem địa chỉ doanh nghiệp",
    sub: "Mở Google Maps",
...
// ── Desktop floating widget (giữ nguyên) ───────────────────────────────────
  ...
  return (
    ...
    >
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="w-[300px] bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100"
          >
            <div className="bg-[#F7a3a9] px-5 py-4">
              ...
              {DESKTOP_ITEMS.map((item, i) => (
                <div key={item.id}>
                  <a
                    ...
                    className="flex items-center gap-3.5 px-4 py-3.5 hover:bg-[#fff0f3] transition-colors duration-200 cursor-pointer group"
                    onClick={() => item.id === "phone" && setOpen(false)}
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${item.iconBg} ${item.iconColor} transition-transform duration-200 group-hover:scale-110`}
                    >
                      {item.icon}
                    </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.94 }}
        className="w-14 h-14 rounded-full bg-[#F7a3a9] text-white shadow-lg shadow-[#F7a3a9]/40 flex items-center justify-center hover:shadow-xl hover:shadow-[#e8547a]/50 transition-shadow duration-300"
      >
        <motion.div
...
const MOBILE_BAR_ITEMS = [
  ...
  },
  {
    ...
    href: `tel:${CONTACT_INFO.phone}`,
    icon: (
      <div className="w-[26px] h-[26px] rounded-full bg-red-500 flex items-center justify-center flex-shrink-0">
...
function MobileBar() {
  return (
    <motion.div
      ...
      animate={{ y: 0 }}
      transition={{ duration: 0.3, ease: "easeOut" }}
      className="fixed bottom-0 left-0 w-full z-50 bg-white border-t border-gray-100 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]"
        ...
        {MOBILE_BAR_ITEMS.map((item, i) => (
          <React.Fragment key={item.id}>
            <a
              ...
              rel="noopener noreferrer"
              className="flex flex-1 items-center justify-center gap-2 py-1.5 rounded-xl active:bg-gray-50 active:scale-95 transition-all duration-150"
            >
              {item.icon}
...
// ── Export: tự chọn layout theo breakpoint ─────────────────────────────────
